...
function App() {
  ...
  const [reportSubTab, setReportSubTab] = React.useState('reports');
  ...
  }, [leftPanelWidth, rightPanelWidth, handleResizeMove, stopResize]);
  ...
  React.useEffect(() => {
    ...
    if (gtfsMapData) engine.setGtfsMapData(gtfsMapData);
    if (gtfsShapesData) engine.setGtfsShapesData(gtfsShapesData);
    engine.start();
    ...
    const loadGtfs = async () => {
      try {
        ...
        if (!cancelled && mapResponse.ok) {
          ...
          if (mapPayload && typeof mapPayload === 'object') setGtfsMapData(mapPayload);
        }
        if (!cancelled && shapesResponse.ok) {
          const shapesPayload = await shapesResponse.json();
          if (shapesPayload && typeof shapesPayload === 'object') setGtfsShapesData(shapesPayload);
        }
      } catch { /* GTFS opcional */ }
    };
    loadGtfs();
  ...
  React.useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      if (newEvents.length > 0) setEventLog((prevLog) => [...newEvents, ...prevLog].slice(0, 200));
  ...
  }, [leftPanelOpen, rightPanelOpen, leftPanelWidth, rightPanelWidth, clampPanelWidth]);

  const navTabClass = (tab) =>
    `text-sm font-bold border-b-2 h-full transition-colors ${currentTab === tab ? 'border-primary text-white' : 'border-transparent text-muted-foreground/60 hover:text-white'}`;

  const subTabClass = (tab) =>
    `text-xs font-bold py-3 border-b-2 transition-colors ${reportSubTab === tab ? 'border-primary text-white' : 'border-transparent text-muted-foreground/50 hover:text-white'}`;
        ...
        <main className="flex flex-1 overflow-hidden relative">
          ...
          {currentTab === 'reports' && (
            <div className="flex flex-col flex-1 overflow-hidden">
              {/* Sub-navigation */}
              <div className="flex items-center gap-8 px-8 border-b border-[#1c1c1f] bg-[#0c0c0e]">
                {REPORT_SUB_TABS.map(({ key, label }) => (
                  <button key={key} className={subTabClass(key)} onClick={() => setReportSubTab(key)}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                {reportSubTab === 'safety' && (
                  <SafetyView metrics={metrics} config={config} />
                )}
                {reportSubTab === 'logs' && (
                  <SystemLogsView
                    metrics={metrics}
